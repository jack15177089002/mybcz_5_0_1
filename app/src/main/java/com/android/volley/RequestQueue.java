        ...
        if (!request.shouldCache()) {
            ...
            synchronized (this.mWaitingRequests) {
                ...
                if (this.mWaitingRequests.containsKey(cacheKey)) {
                    ...
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m2464v("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
                    }
                } else {
    ...
    public void finish(Request<?> request) {
        ...
        if (request.shouldCache()) {
            synchronized (this.mWaitingRequests) {
                ...
                if (queue != null) {
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m2464v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(queue.size()), cacheKey);
                    }
