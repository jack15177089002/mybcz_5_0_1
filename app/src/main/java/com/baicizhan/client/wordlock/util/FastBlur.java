...
public class FastBlur {
    ...
    class BlurTask extends AsyncTask<Bitmap, Void, Bitmap> {
        ...
        BlurTask(ImageView imageView) {
            this.mTarget = new WeakReference<>(imageView);
        }

        /* access modifiers changed from: protected */
        public Bitmap doInBackground(Bitmap... bitmapArr) {
            return FastBlur.doBlur(bitmapArr[0], 16, true);
        }
    ...
    public static void applyBlur(final Activity activity, final View view) {
        if (activity != null && view != null) {
            view.getViewTreeObserver().addOnPreDrawListener(new OnPreDrawListener() {
                public final boolean onPreDraw() {
                    view.getViewTreeObserver().removeOnPreDrawListener(this);
                    view.buildDrawingCache();
                    FastBlur.blur(activity, ((BitmapDrawable) activity.getWallpaper()).getBitmap(), view);
                    return true;
                }
            });
        }
    ...
    public static void blur(Context context, Bitmap bitmap, View view) {
        Bitmap cropSameScale = BitmapUtil.cropSameScale(bitmap.copy(Config.ARGB_8888, false), Common.getScreenWidth(context), Common.getScreenHeight(context), 2);
        Bitmap createScaledBitmap = Bitmap.createScaledBitmap(cropSameScale, Common.getScreenWidth(context), Common.getScreenHeight(context), false);
        if (createScaledBitmap != cropSameScale) {
            cropSameScale.recycle();
        }
        int measuredWidth = (int) (((float) view.getMeasuredWidth()) / 6.0f);
        int measuredHeight = (int) (((float) view.getMeasuredHeight()) / 6.0f);
        if (measuredWidth <= 0 || measuredHeight <= 0) {
            if (view.getMeasuredWidth() <= 0 || view.getMeasuredHeight() <= 0) {
                C1604j.m3631a(view, new BitmapDrawable(view.getContext().getResources(), createScaledBitmap));
                return;
            } else {
        ...
        canvas.translate(((float) (-view.getLeft())) / 6.0f, ((float) (-view.getTop())) / 6.0f);
        canvas.scale(0.16666667f, 0.16666667f);
        Paint paint = new Paint();
        paint.setFlags(2);
        canvas.drawBitmap(createScaledBitmap, 0.0f, 0.0f, paint);
        createScaledBitmap.recycle();
        C1604j.m3631a(view, new BitmapDrawable(view.getContext().getResources(), doBlur(createBitmap, 16, true)));
    }
}
